// javascript Created by Dumindu Induwra Gamage dumindu.20221168@iit.ac.lk

// This code line retrieves the form elements as JavaScript objects for the personal details section.
...
// This code line retrieves the form elements as JavaScript objects for the billing details section.
...
let btn_order_now = document.getElementsByClassName("order_now btn btn-primary");       // This will get all elements with classes order_now AND btn AND btn-primary
let btn_add_to_cart = document.getElementsByClassName("add_to_cart btn btn-primary");   //This will  get all elements with classes add_to_cart AND btn AND btn-primary

// This code line retrieves the shopping cart table element as a JavaScript object.

let shopping_cart_table = document.getElementById("shopping_cart_table");

// These variables hold the order details
let pro_name=null;
let pro_amount=null;
let pro_price=null;
let pro_detail=null;
...
// In here,event listeners defined 
btn_place_order.addEventListener("click",validate_info);          // btn_place_order button in personal details section.

// The cart array stores the items.
let cart_arr=[];

// This code block pertains to the "Order Now" buttons in the products list.
for(let i = 0; i < btn_order_now.length; i++) 
{
    btn_order_now[i].addEventListener("click", function()         // This is a anonymous inner function.
    {
   
        console.log("Order Button Clicked");
        if(validate_product_amount(parseInt(amount[i].value), i)) // If the user's input is not numerical, the parseInt() function will return a NaN (Not a Number) property.
        {                                                        // If the user enters a float value, the parseInt() function will truncate the fraction part and return an integer value
...
//_________________________________________________________________Order some producr starts here______________________________________________________________________________
...
//____________________________________________________________________Order some producrts ends here_________________________________________________________________________________

//_____________________________________________________________________Add to cart starts here._________________________________________________________________________________
...
function remove_from_cart()
{
    ...
    {
        td_remove_item_btn[i].addEventListener("click", function()            // this is a anonymous inner function.
        {
...
//_____________________________________________________________________Add to cart section ends here.__________________________________________________________________________________
